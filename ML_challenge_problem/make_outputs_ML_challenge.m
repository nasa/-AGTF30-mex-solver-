...
outputs_challenge_problem(input_num).altitude = altitude_sensed;
outputs_challenge_problem(input_num).mach_number = mach_number_sensed;

outputs_challenge_problem(input_num).U = [ 
    U(1);                       % wf
    solver_initial_guess(9);    % VAFN
    solver_initial_guess(10)];  % VBV

outputs_challenge_problem(input_num).Y = [ 
